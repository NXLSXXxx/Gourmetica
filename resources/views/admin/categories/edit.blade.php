...
@section('title', 'Editar Categoría | Gourmetica Intranet')
...
<div class="max-w-2xl mx-auto">
    <header class="mb-12">
        ...
        <h1 class="text-3xl font-serif font-bold text-white tracking-tight">Editar Categoría</h1>
        <p class="text-slate-400 mt-2">Modifica los detalles de la categoría de productos.</p>
    </header>
    ...
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 bg-brand-primary p-8 rounded-2xl border border-slate-700 shadow-xl">
        @csrf
        @method('PUT')
        
        <div>
            <label class="block text-sm font-medium text-slate-400 mb-2">Nombre de la Categoría</label>
            <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" required class="w-full bg-slate-800 border border-slate-600 rounded-lg px-4 py-3 text-white focus:border-brand-primary outline-none transition-all" placeholder="Ej: Tortas de Boda">
        </div>
        
        <div>
            <label class="block text-sm font-medium text-slate-400 mb-2">URL Slug (Identificador)</label>
            <input type="text" name="slug" id="slug" value="{{ old('slug', $category->slug) }}" required class="w-full bg-slate-800 border border-slate-600 rounded-lg px-4 py-3 text-white focus:border-brand-primary outline-none transition-all" placeholder="ej-tortas-boda">
        ...
        <div>
            ...
            @if($category->image)
                <div class="mb-4">
                    ...
                    <img src="{{ asset('storage/' . $category->image) }}" class="h-32 w-auto object-cover rounded-xl border border-slate-700 shadow-md">
                </div>
            @endif
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        nameInput.addEventListener('input', function() {
            // Only auto-slug if user hasn't heavily modified slug manually
